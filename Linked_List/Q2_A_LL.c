...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2) {
  ...
  int mergeCnt = ll1->size < ll2->size ? ll1->size : ll2->size;
  int insertIdx = 1;
  ListNode *LL2Cur;

  // ll1에 ll2의 노드를 merge할 node가 없으면 merge하지 않는다
  if (ll1->head == NULL || ll2->head == NULL) return;

  for (int i = 0; i < mergeCnt; i++) {
    LL2Cur = ll2->head;
    insertNode(ll1, insertIdx, LL2Cur->item);
    removeNode(ll2, 0);
    insertIdx += 2;
  }
